        ...
        if isinstance(cand, variable.VariableNode):
            ...
            if creator is not None and (creator, cand) not in seen_edges:
                ...
                if cand.name is None:
                    # VariableNode with creator and name means internal Variable
                    creator_id = str(creator).split(' ')[-1].replace('>','')
                    calc_var.append([creator.label, creator_id ,cand.rank,cand.shape,cand])
                else:
                    print(cand.name)
            elif (creator, cand) not in seen_edges:
                if cand.name is None:
                    # no creator and no name VariableNode means PrimitiveInput
                    cand_id = str(cand).split(' ')[-1].replace('>','')
                    calc_var.append(["PrimitiveIn",cand_id, cand.rank,cand.shape,cand])
        elif isinstance(cand, function_node.FunctionNode):
            for input_ in cand.inputs:
                #print("FunctionNode",input_.name,input_.rank)
                if input_.name is not None:
                    print(input_.name, input_.rank, type(input_.data))
                    if input_.data is not None:print(input_.data.shape)
